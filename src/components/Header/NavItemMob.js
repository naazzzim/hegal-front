...
const NavItemMob = ({item}) => {
  ...
  return (
    <>
        <div className={styles.navItemMob}><Link href={item[0].route}>{item[0].name}</Link><span onClick={()=>toggleOpen()}>&nbsp;{item.length>1?<Image src={'/icons/down.svg'} alt="arrow" height={10} width={10} />:''}</span></div>
